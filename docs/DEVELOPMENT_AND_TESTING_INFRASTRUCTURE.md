...
The repository already contains an important piece of long-term infrastructure: `src/TwoRivers.Berelain/package.json` is used to restore the full Font Awesome asset set for development and testing in a reproducible way.
...
The current Font Awesome package references live under `dependencies` in `src/TwoRivers.Berelain/package.json`.
...
4. `Sync-BerelainAssets.ps1`

These should wrap common workflows without hiding what they do.

### Asset Sync Script Responsibilities

`Sync-BerelainAssets.ps1` should:
...
### Unit Test Framework and Libraries
...
### Integration Test Framework and Libraries

- `xunit.v3`
- `Moq` where a dependency must be substituted inside the host
- `FluentAssertions`

### Integration Test Host Model
...
### Integration Test Reset Strategy
...
1. `Berelain.Development.recipe.json`
   - local manual development convenience
2. `Berelain.Integration.recipe.json`
